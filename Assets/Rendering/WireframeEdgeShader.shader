...
Shader "MogwaiProcedural/WireframeEdgeShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void geom(line vs_out input[2], inout TriangleStream<vs_out> OutputStream)
            {
                ...
                float animatedSphereRadius = _PointSphereRadius;

                // Don't use animatedSphereRadius as we're not animating across length
                float3 shortenAmount = min(length(baseAxisRay) / 3, 3 * _VertexSphereRadius);
                float4 p0WorldShortened = float4(p0World.xyz + axisRay * shortenAmount, 1);
                float4 p1WorldShortened = float4(p1World.xyz - axisRay * shortenAmount, 1);
                ...
                float3 avgNormal = normalize(float3(0, 1, 0));
                ...
                float4 frontUpLeft = float4(p0WorldShortened + scaledRHat + scaledHHat - axisRay * _PointSphereRadius,
                1);
                float4 frontUpRight = float4(p0WorldShortened - scaledRHat + scaledHHat - axisRay * _PointSphereRadius,
                            1);
                float4 frontDownLeft = float4(p0WorldShortened + scaledRHat - scaledHHat - axisRay * _PointSphereRadius,
                                                    1);
                float4 frontDownRight = float4(
                    p0WorldShortened - scaledRHat - scaledHHat - axisRay * _PointSphereRadius, 1);

                float4 backUpLeft = float4(
                    p0WorldShortened + scaledRHat + scaledHHat + scaledAHat + axisRay * _PointSphereRadius, 1);
                float4 backUpRight = float4(
                    p0WorldShortened - scaledRHat + scaledHHat + scaledAHat + axisRay * _PointSphereRadius, 1);
                float4 backDownLeft = float4(
                    p0WorldShortened + scaledRHat - scaledHHat + scaledAHat + axisRay * _PointSphereRadius, 1);
                float4 backDownRight = float4(
                    p0WorldShortened - scaledRHat - scaledHHat + scaledAHat + axisRay * _PointSphereRadius, 1);
   ...
   normalize(frontUpLeft.xyz - frontDownLeft.xyz)), 0);
            ...
            fixed4 frag(vs_out i) : SV_Target
            {
                ...
                //Radius alpha
                float diff = length(worldPos - _SelectPositionWorld);
                ...
                return animPct * float4(lightOut, alphaMult * _MaterialColor.a);
            }
            ENDCG
        }
    }
